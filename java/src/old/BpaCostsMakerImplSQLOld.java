...
public class BpaCostsMakerImplSQLOld implements BpaCostsMakerOld {

	private PeriodMakerOld periodMaker;
	
	private File bpaCosts;
		
	public BpaCostsMakerImplSQLOld(PeriodMakerOld periodMaker){
		this.periodMaker=periodMaker;
	}
	
	public PeriodMakerOld getPeriodMaker(){
	...
	public boolean extractGL(Map<String,File> periodFiles,Map<String,String> driversMap ){
				 ...
				 //BufferedReader in = new BufferedReader(new FileReader(periodFiles.get(periodMaker.getConfiguration().getGLFile().getName())));)
		{
			...
			try {
				...
				st = ((ConfigurationManagerSQLOld)periodMaker.getConfiguration()).getGLConnection().createStatement();
				...
				rs = st.executeQuery("SELECT * FROM `"+((ConfigurationManagerSQLOld)periodMaker.getConfiguration()).getGLConnectionSettings().get(((ConfigurationManagerSQLOld)periodMaker.getConfiguration()).getGLConnectionSettings().size()-1)+"`");
